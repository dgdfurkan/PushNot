...
const state={
  settings:{...DEFAULTS,...JSON.parse(localStorage.getItem('vakit.settings')||'{}')},
...
async function loadPrayer(){
  ...
  try{
    ...
    const r=await fetch('/api/prayer-times?'+q); if(!r.ok)throw new Error();
    const j=await r.json(); state.prayer=j.timings; state.prayer.demo=false; $('#statusText').textContent=`Vakitler güncel · ${j.source||'sunucu'}`;
  }catch{state.prayer=demoPrayer();$('#statusText').textContent='Sunucuya ulaşılamadı · önizleme verisi gösteriliyor'}
...
function buildPlan(){
  ...
  let nap=core<330?{start:addMin(napEnd,-90),end:napEnd,minutes:90}:core<420?{start:addMin(napEnd,-20),end:napEnd,minutes:20}:{start:addMin(napEnd,-20),end:napEnd,minutes:20};
...
function renderPrayer(){
  const p=state.prayer,rows=[['Sabah',p.Fajr||p.Imsak],['Öğle',p.Dhuhr],['İkindi',p.Asr],['Akşam',p.Maghrib],['Yatsı',p.Isha]],now=new Date();
  let active=rows.findIndex(([,t])=>parseTime(clean(t))>now);if(active<0)active=4;
  $('#prayerGrid').innerHTML=rows.map(([n,t],i)=>`<div class="prayer-pill ${i===active?'active':''}"><span>${n}</span><strong>${clean(t)}</strong></div>`).join('');
}
function nextEvents(){
  const p=state.prayer,x=state.plan;
  return [
    {t:x.wake,title:'Uyanma zamanı',text:`Güneşe ${state.settings.wakeOffset} dk var. Sabah namazı için kalk.`,icon:'sun'},
    {t:x.activityStart,title:'Dışarı çık',text:state.settings.activity==='walk'?'30 dk yürüyüş zamanı.':'40 dk bisiklet zamanı.',icon:'walk'},
    ...[['Dhuhr','Öğle'],['Asr','İkindi'],['Maghrib','Akşam'],['Isha','Yatsı']].map(([k,n])=>({t:addMin(parseTime(clean(p[k])),-5),title:`${n} namazı yaklaşıyor`,text:'5 dakika kaldı.',icon:'pray'}))
  ].sort((a,b)=>a.t-b.t);
}
function renderHero(){const now=new Date(),ev=nextEvents(),e=ev.find(x=>x.t>now)||ev.at(-1),diff=mins(now,e.t);$('#heroIcon').innerHTML=ICONS[e.icon];$('#heroTitle').textContent=e.title;$('#heroText').textContent=e.text;$('#heroTime').textContent=fmt(e.t);$('#heroEyebrow').textContent=diff>=0?`${diff} dk sonra`:'Bugünün son adımı'}
function renderTimeline(){
  const p=state.prayer,x=state.plan,rows=[
    ...
    [fmt(x.breakfast),'food','Kahvaltı','Eve dönüşte öneri hazır'],
    [clean(p.Dhuhr),'pray','Öğle namazı','5 dk önce bildirim'],[clean(p.Asr),'pray','İkindi namazı','5 dk önce bildirim'],[clean(p.Maghrib),'pray','Akşam namazı','5 dk önce bildirim'],[clean(p.Isha),'pray','Yatsı namazı','5 dk önce bildirim']
  ];
  if(state.settings.workDay)rows.push(['20:00','work','İşe hazırlan','İş günü bildirimi açık']);
  $('#timeline').innerHTML=rows.map(r=>`<div class="timeline-row"><div class="timeline-time">${r[0]}</div><div class="timeline-icon">${ICONS[r[1]]}</div><div class="timeline-copy"><strong>${r[2]}</strong><span>${r[3]}</span></div></div>`).join('');
}
function renderSleep(){
  ...
  $('#sleepTitle').textContent=`Gece uykusu ${h} sa ${m} dk`;
  $('#sleepText').textContent=x.core<330?(x.nap?`${fmt(x.nap.start)}–${fmt(x.nap.end)} arasında 90 dk telafi bloğu öneriliyor. 20 dk tek başına bu açığı kapatmaz.`:'Bugün telafi uykusunu kapattın; mümkünse geceyi daha erkene çek.'):(x.nap?`${fmt(x.nap.start)}–${fmt(x.nap.end)} kısa şekerleme penceresi.`:'Şekerleme bugün kapalı.');
...
const breakfasts=[['Yumurta + peynir + domates','Kekik ve baharat ekle; yanına su. Ekmek varsa küçük porsiyon.'],['Yoğurt + yulaf + meyve','Evde varsa ceviz/badem ekle.'],['Peynirli omlet + domates','Yanında yeşillik varsa ekle.'],['Avokado + yumurta + domates','Ekmeği küçük tut.'],['Peynir + domates + zeytin + yoğurt','Yumurtasız gün için basit tabak.']];let breakfastIndex=0;
...
function renderSettings(){
  $('#todayLabel').textContent=new Intl.DateTimeFormat('tr-TR',{weekday:'long',day:'numeric',month:'long'}).format(new Date());$('#locationTitle').textContent=`${state.settings.district}, ${state.settings.city}`;
...
function renderAll(){renderPrayer();renderHero();renderTimeline();renderSleep();renderBreakfast();renderSettings();renderStats()}
function schedulePayload(){
  const p=state.prayer,x=state.plan,today=new Date(),ev=[];const add=(t,title,body,tag)=>{if(t>Date.now())ev.push({at:new Date(t).toISOString(),title,body,tag})};
  add(x.wake,'Uyanma zamanı',`Güneşe ${state.settings.wakeOffset} dk var. Sabah namazı için kalk.`,'wake');
  add(x.activityStart,'Güneş doğdu',state.settings.activity==='walk'?'Yürüyüş zamanı.':'Bisiklet zamanı.','activity-start');
  [['Dhuhr','Öğle'],['Asr','İkindi'],['Maghrib','Akşam'],['Isha','Yatsı']].forEach(([k,n])=>add(addMin(parseTime(clean(p[k]),today),-5),`${n} namazına 5 dk`,`${n} vakti ${clean(p[k])}.`,'prayer-'+k));
  if(x.nap)add(addMin(x.nap.start,-5),'Şekerleme penceresi',`${fmt(x.nap.start)}’te ${x.nap.minutes} dk uyku planın var.`,'nap');
  if(state.settings.workDay)add(parseTime('19:30',today),'İşe hazırlık','30 dakika sonra iş için çıkış hazırlığı.','work');return ev;
}
...
async function syncPushSchedule(){
  ...
  try{const reg=await navigator.serviceWorker.ready,sub=await reg.pushManager.getSubscription();if(!sub)return;const profile={district:state.settings.district,city:state.settings.city,country:'TR',wakeOffset:Number(state.settings.wakeOffset),activity:state.settings.activity,activityMinutes:Number(state.settings.activityMinutes[state.settings.activity])};await fetch('/api/profile',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({subscription:sub.toJSON(),profile})});await fetch('/api/schedule',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({subscription:sub.toJSON(),events:schedulePayload()})})}catch{}
}
async function enableNotifications(){
  ...
  try{if(await Notification.requestPermission()!=='granted')throw new Error();const reg=await navigator.serviceWorker.ready;let sub=await reg.pushManager.getSubscription();if(!sub){const k=await fetch('/api/vapid-public-key').then(r=>r.json());sub=await reg.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:b64url(k.publicKey)})}$('#notificationNote').textContent='Bildirimler açık. Plan push sunucusuyla eşitlendi.';await syncPushSchedule();toast('Bildirimler açıldı.')}catch{$('#notificationNote').textContent='Bildirim kurulamadı. PWA olarak ana ekrandan açtığından emin ol.';toast('Bildirim kurulamadı.')}
}
async function testNotification(){
  if(Notification.permission!=='granted'){toast('Önce bildirimleri aç.');return}
  try{const reg=await navigator.serviceWorker.ready,sub=await reg.pushManager.getSubscription();if(!sub)throw new Error();await fetch('/api/test-push',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({subscription:sub.toJSON(),message:'Test başarılı. Vakit bildirimleri çalışıyor.'})});toast('Test gönderildi.')}catch{toast('Test gönderilemedi.')}
}
...
function startActivity(){if(state.activity?.running)return;const total=state.settings.activityMinutes[state.settings.activity];state.activity={running:true,type:state.settings.activity,total,end:Date.now()+total*60000};$('#startActivity').classList.add('hidden');$('#activityLive').classList.remove('hidden');tickActivity();state.activity.timer=setInterval(tickActivity,1000);scheduleActivityPush(total)}
...
function finishActivity(){if(!state.activity)return;clearInterval(state.activity.timer);const done=Math.max(1,Math.min(state.activity.total,Math.round((state.activity.total*60000-(state.activity.end-Date.now()))/60000)));state.stats.activity={type:state.activity.type,minutes:done};state.activity=null;$('#activityLive').classList.add('hidden');$('#startActivity').classList.remove('hidden');saveStats();toast('Aktivite kaydedildi. Kahvaltı zamanı.')}
async function scheduleActivityPush(minutes){if(Notification.permission!=='granted')return;try{const reg=await navigator.serviceWorker.ready,sub=await reg.pushManager.getSubscription();if(!sub)return;const now=Date.now(),events=[{at:new Date(now+(minutes-5)*60000).toISOString(),title:'5 dakika kaldı',body:'Yavaştan dönüşe geç.',tag:'activity-five'},{at:new Date(now+minutes*60000).toISOString(),title:'Aktivite tamamlandı',body:'Eve dön. Kahvaltını hazırlayalım.',tag:'activity-end'}];await fetch('/api/schedule',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({subscription:sub.toJSON(),events,append:true})})}catch{}}
function renderFocus(){const f=state.focus,total=(f.mode==='focus'?f.preset:(f.preset===25?5:10))*60,pct=1-f.remaining/total;$('#focusModeLabel').textContent=f.mode==='focus'?'Odak':'Mola';$('#focusTimer').textContent=`${String(Math.floor(f.remaining/60)).padStart(2,'0')}:${String(f.remaining%60).padStart(2,'0')}`;$('#focusRing').style.background=`conic-gradient(var(--sage) ${pct*360}deg,var(--surface-2) 0deg)`;$('#focusToggle').textContent=f.running?'Duraklat':'Başlat'}
function focusToggle(){const f=state.focus;f.running=!f.running;if(f.running){f.last=Date.now();f.timer=setInterval(()=>{const now=Date.now();f.remaining-=Math.max(1,Math.floor((now-f.last)/1000));f.last=now;if(f.remaining<=0){clearInterval(f.timer);if(f.mode==='focus'){state.stats.focus=(state.stats.focus||0)+f.preset;state.stats.rounds=(state.stats.rounds||0)+1;f.mode='break';f.remaining=(f.preset===25?5:10)*60}else{state.stats.break=(state.stats.break||0)+(f.preset===25?5:10);f.mode='focus';f.remaining=f.preset*60}f.running=false;saveStats();toast(f.mode==='break'?'Odak tamam. Mola zamanı.':'Mola bitti. Yeni tura hazırsın.')}renderFocus()},1000)}else clearInterval(f.timer);renderFocus()}
...
function bind(){
  $$('.nav-item').forEach(b=>b.onclick=()=>{$$('.nav-item').forEach(x=>x.classList.toggle('active',x===b));$$('.page').forEach(p=>p.classList.toggle('active',p.dataset.page===b.dataset.nav))});
  $$('[data-goto]').forEach(b=>b.onclick=()=>document.querySelector(`[data-nav="${b.dataset.goto}"]`).click());
...
async function init(){if(state.settings.workDayDate!==todayKey()){state.settings.workDay=true;state.settings.workDayDate=todayKey();saveSettings()}if('serviceWorker'in navigator)await navigator.serviceWorker.register('/sw.js');bind();notificationStatus();renderFocus();renderSettings();renderStats();await loadPrayer();setInterval(()=>{if(state.prayer){renderHero();renderPrayer()}},30000)}
init();
